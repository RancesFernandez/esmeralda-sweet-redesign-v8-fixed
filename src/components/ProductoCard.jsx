...
export default function ProductoCard({ producto, categoria }) {
  ...
  const categoryLabel = producto.categoria === 'dulce'
    ? 'Propuesta dulce'
  ...
  return (
    <>
      ...
      {open && (
        <div className="modal-backdrop product-modal-backdrop" onClick={() => setOpen(false)}>
          <div className="modal-product" role="dialog" aria-modal="true" aria-labelledby={`producto-${producto.id}`} onClick={(event) => event.stopPropagation()}>
            <button className="modal-close" onClick={() => setOpen(false)} aria-label="Cerrar detalle">×</button>
            <img src={producto.imagen} alt={producto.nombre} />
            <div>
              <p className="section-kicker">{categoria || (producto.categoria === 'dulce' ? 'Menú dulce' : producto.categoria === 'salado' ? 'Menú salado' : 'Desayunos')}</p>
